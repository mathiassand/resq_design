...
country_mean_df <- join_df %>%
  ...
  filter(country == "Country #54", runningYear >= 1, runningYear <= 5) %>%
  summarise(countryMean = mean(DTNMean)) %>% ungroup()
...
cohort_df %>%
  ...
  geom_point() +
  geom_smooth(se = TRUE, alpha = .5) +
  geom_smooth(data = country_mean_df, aes(x = jitter(runningYear, amount = 0.1), colour = countryMean, y=countryMean), se = TRUE, alpha = .5) +
  theme_bw() +
  coord_cartesian(ylim = c(0, 200)) +
  scale_x_continuous(breaks = seq(1, 6, 1)) + labs(title="Hospitals in country 54, Including 2021")
...
cohort_df %>%
  ...
  theme_bw() +
  coord_cartesian(ylim = c(0, 200)) +
  scale_x_continuous(breaks = seq(1, 6, 1)) + labs(title="Hospitals in country 54, Excluding 2021")
```
...
```{r countryPlotsv2, echo = FALSE}
country54_hospitals_df<-resq_clean %>%
  filter(country == "Country #54" & year > 2015) %>%
  group_by(hospital) %>%
  summarise(
    hospitalFirstYear = min(year),
    
  )

country54_df<-resq_clean %>%
  filter(country == "Country #54" & year > 2015) %>%
  group_by(country) %>%
  summarise(country54DTNMean = 
